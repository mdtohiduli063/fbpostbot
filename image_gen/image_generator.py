...
class ImageGenerator:
    ...
    def generate(self, headline: str, body: str = "",
        ...
        try:
            ...
            badge_bot = self._draw_badge(draw, img, badge_text,
                                         x=pad, y=38, accent=accent)

            # 4. Headline block — large, wrapped, left-aligned
            hl_top = badge_bot + 22
            headline_bot = self._draw_headline(img, headline.strip() or "—",
                                               x=pad, top=hl_top,
                                               max_w=content_w)
            ...
            # 7. Bottom brand bar
            self._draw_bottom_bar(draw, img, accent, palette,
    ...
    def _make_background(self, palette: Tuple[RGB, RGB, RGB],
                         accent: RGB) -> Image.Image:
        ...
        for y in range(0, self.height, 80):
            od2.line([(0, y), (self.width, y)],
                     fill=(255, 255, 255, 6), width=1)
    ...
    def _draw_badge(self, draw: ImageDraw.ImageDraw, img: Image.Image,
                    text: str, x: int, y: int, accent: RGB) -> int:
        ...
        self._bn_render(img, text, x + pad_x, y + pad_y, size, fill=text_fill,
                        shadow=False)
    ...
    def _draw_headline(self, img: Image.Image, text: str,
                       x: int, top: int, max_w: int) -> int:
        ...
        for i, ln in enumerate(lines):
            ...
            self._bn_render(img, ln, x=x, y=y, size=size,
                            fill=(255, 255, 255),
    ...
    def _draw_divider(self, draw: ImageDraw.ImageDraw, accent: RGB,
                      y: int) -> None:
        """Horizontal accent divider line with fade-out ends."""
        pad = self.padding
        # Main line
        draw.line([(pad, y), (self.width - pad, y)],
                  fill=(*accent, 200), width=2)
        # Subtle glow above
        draw.line([(pad, y - 1), (self.width - pad, y - 1)],
                  fill=(*accent, 60), width=1)

    # ─────────────────────────── body ───────────────────────────────────────

    def _draw_body(self, img: Image.Image, body: str,
                   top: int, bot: int, max_w: int) -> None:
        ...
        import re
        body = re.sub(r'^[\U00010000-\U0010ffff\U00002600-\U000027BF\U0001F300-\U0001FAFF]\s*', '', body)
        body = re.sub(r'^\W+\s*\w+[：:]\s*', '', body)  # remove "💰 দাম-দর সংক্রান্ত খবর:" prefix style
        ...
        for i, ln in enumerate(lines):
            y = start_y + i * line_h
            self._bn_render_centered(img, ln, y=y, size=size,
                                     fill=(232, 236, 245),
                                     shadow=True, shadow_offset=2)

    # ─────────────────────────── bottom bar ─────────────────────────────────

    def _draw_bottom_bar(self, draw: ImageDraw.ImageDraw, img: Image.Image,
        ...
        draw.rectangle([0, bar_top, self.width, bar_top + 3],
                       fill=(*accent, 210))
        ...
        self._bn_render(img, brand, x=bx, y=by, size=brand_size,
                        fill=(220, 225, 240),
                        shadow=False)
        ...
        dot_r = 4
        if bx > 40:
            draw.ellipse([bx - 28 - dot_r, dot_y - dot_r,
                          bx - 28 + dot_r, dot_y + dot_r],
                         fill=(*accent, 200))
    ...
    def _bn_render(self, img: Image.Image, text: str, x: int, y: int,
        ...
        if self._renderer:
            return self._renderer.render(
                ...
                shadow_fill=(0, 0, 0), shadow_alpha=190,
            )
        d = ImageDraw.Draw(img, "RGBA")
        font = self._font(size)
        if shadow:
            d.text((x + shadow_offset, y + shadow_offset), text,
                   fill=(0, 0, 0, 190), font=font)
